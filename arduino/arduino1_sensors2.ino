...
// === ESTADO ===
bool autoIrrigation = true;  // ← ATIVADO POR PADRÃO!
bool coolerOn = false;
bool lightOn = false;
...
void readAndSendSensorData() {
  ...
  // 1. IRRIGAÇÃO AUTOMÁTICA
  // Se umidade do solo < mínimo configurado → Liga bomba
  if (autoIrrigation && soilPercent < thresholds.terraMin) {
    activatePump(3000);  // Irriga por 3 segundos
    Serial.println("{\"action\":\"pump_auto_on\",\"reason\":\"low_soil\",\"value\":" + String(soilPercent) + "}");
  }
  
  // 2. COOLER AUTOMÁTICO
  // Se temperatura > máximo configurado → Liga cooler
  if (temp > thresholds.tempMax && !coolerOn) {
    setCooler(true);
    Serial.println("{\"action\":\"cooler_auto_on\",\"reason\":\"high_temp\",\"value\":" + String(temp, 1) + "}");
  } 
  // Se temperatura < (máximo - 2°C) → Desliga cooler (histerese)
  else if (temp < thresholds.tempMax - 2 && coolerOn) {
    setCooler(false);
    Serial.println("{\"action\":\"cooler_auto_off\",\"reason\":\"temp_normal\",\"value\":" + String(temp, 1) + "}");
  }
  
  // 3. FITA LED AUTOMÁTICA
  // Se luminosidade < mínimo configurado → Liga LED
  if (ldrPercent < thresholds.luzMin && !lightOn) {
    setLight(true);
    Serial.println("{\"action\":\"light_auto_on\",\"reason\":\"low_light\",\"value\":" + String(ldrPercent) + "}");
  } 
  // Se luminosidade > (mínimo + 10%) → Desliga LED (histerese)
  else if (ldrPercent > thresholds.luzMin + 10 && lightOn) {
    setLight(false);
    Serial.println("{\"action\":\"light_auto_off\",\"reason\":\"light_normal\",\"value\":" + String(ldrPercent) + "}");
  }
